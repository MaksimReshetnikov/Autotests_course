...
def move_zeros(lst):
    my_lst_without_zeros = []
    for i in lst:
        if i != 0:
            my_lst_without_zeros.append(i)
    quantity_zero = len(lst) - len(my_lst_without_zeros)
    if quantity_zero == 0:
        return lst
    else:
        for j in range(quantity_zero):
            my_lst_without_zeros.append(0)
    return my_lst_without_zeros
